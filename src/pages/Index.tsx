...
const Index = () => {
  ...
  const scenario = useMemo(() => scenarios.find((s) => s.id === activeScenarioId) ?? scenarios[0], [activeScenarioId]);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="border-b border-border/50 px-4 py-3 flex items-center justify-between bg-card/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 text-[10px] font-mono text-muted-foreground">
          ...
          <span className="hidden sm:inline">{scenarios.length} SCENARIOS LOADED</span>
        </div>
      </header>
